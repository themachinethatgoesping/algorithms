...
TEST_CASE("find_local_maxima2 in 3D", TESTTAG)
{
    ...
    struct Peak { int64_t x, y, z; float value; };
    std::vector<Peak> peaks = { {10,20,30,6.5f},{25,25,25,7.2f},{30,15,35,8.8f},
                                {35,35,20,9.1f},{40,40,45,10.5f} };
    for (auto& p : peaks) data(p.x, p.y, p.z) += p.value;
    ...
    for (size_t i = 0; i < maxima.size(); ++i)
    {
        ...
        info_str += fmt::format("Peak {}: x={}, y={}, z={}, value={}\n",
                                i, maxima[i][0], maxima[i][1], maxima[i][2], val);
    }
    INFO(info_str);
...
TEST_CASE("find_local_maxima2 in 2D", TESTTAG)
{
    ...
    struct Peak { int64_t x, y; float value; };
    std::vector<Peak> peaks = { {10,20,6.5f},{25,25,7.2f},{30,15,8.8f},
                                {35,35,9.1f},{40,40,10.5f} };
    for (auto& p : peaks) data(p.x, p.y) += p.value;
    ...
    for (size_t i = 0; i < maxima.size(); ++i)
    {
        ...
        info_str += fmt::format("Peak {}: x={}, y={}, value={}\n",
...
TEST_CASE("find_local_maxima2 in 1D", TESTTAG)
{
    ...
    struct Peak { int64_t x; float value; };
    std::vector<Peak> peaks = { {10,6.5f},{25,7.2f},{30,8.8f},{35,9.1f},{40,10.5f} };
    for (auto& p : peaks) data(p.x) += p.value;
